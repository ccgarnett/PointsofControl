...
/** Mirrors JWT middleware: tests pass ?user_id= to populate req.user */
const injectTestUser: express.RequestHandler = (req, _res, next) => {
  const q = req.query.user_id as string | undefined;
  const authReq = req as AuthRequest;
  if (q !== undefined) {
    authReq.user = { id: q, username: 'test', role: 'User' };
  }
  next();
};
...
describe('createTask', () => {
  ...
  it('returns 401 when user_id is invalid', async () => {
...
describe('readTask', () => {
  ...
  it('returns 401 for invalid user_id', async () => {
